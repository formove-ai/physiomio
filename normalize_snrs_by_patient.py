...
def normalize_snrs_by_patient(input_file='detailed_snrs_table.csv',
    ...
    for patient_id in sorted(df['Patient'].unique())[:3]:
        ...
        print(f"\nPatient {patient_id}:")
        print(
            f"  Original SNR Mean range: [{patient_data[snr_mean_col].min():.2f}, {patient_data[snr_mean_col].max():.2f}]")
        print(f"  Normalized SNR Mean range: [{patient_norm[f'{snr_mean_col} (Normalized)'].min(
        ):.2f}, {patient_norm[f'{snr_mean_col} (Normalized)'].max():.2f}]")
        print(
            f"  Original SNR Std Dev range: [{patient_data[snr_std_col].min():.2f}, {patient_data[snr_std_col].max():.2f}]")
        print(f"  Normalized SNR Std Dev range: [{patient_norm[f'{snr_std_col} (Normalized)'].min(
        ):.2f}, {patient_norm[f'{snr_std_col} (Normalized)'].max():.2f}]")
