...
def normalize_psd_by_patient(input_file='detailed_psd_table.csv',
    ...
    for col in psd_columns[:2]:  # Show first 2 PSD columns as examples
        print(f"  {col}:")
        print(
            f"    Original range: [{patient1_data[col].min():.2e}, {patient1_data[col].max():.2e}]")
        print(f"    Normalized range: [{patient1_norm[f'{col} (Normalized)'].min():.2f}, {
              patient1_norm[f'{col} (Normalized)'].max():.2f}]")
